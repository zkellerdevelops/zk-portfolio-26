...
# ---------- shared partials ----------
def fonts_and_head(title, img_prefix, extra_meta=""):
...
{extra_meta}  <link rel="shortcut icon" type="image/png" href="{img_prefix}{SITE['favicon']}" />
  <link rel="apple-touch-icon" href="{img_prefix}{SITE['apple_icon']}" />
  ...
  <link rel="stylesheet" href="{css_prefix(img_prefix)}assets/css/style.css" />
</head>
"""


def css_prefix(img_prefix):
    # img_prefix is "assets/img/" (home) or "../../assets/img/" (work pages)
    return "" if img_prefix.startswith("assets") else "../../"


def footer(link_prefix, img_prefix):
    return f"""  <footer class="footer-wrap">
    <a href="{link_prefix}" class="footer-link">
      <img src="{img_prefix}{SITE['logo']}" width="15" alt="ZLK logo" class="footer-logo" />
...
def build_home(projects):
  ...
  <meta property="og:image" content="{ip}{meta['og_image']}" />
  <meta property="og:type" content="website" />
  <meta name="twitter:card" content="summary_large_image" />
"""
    social = "\n".join(
        f'          <a href="{s["url"]}" target="_blank" rel="noopener" class="link-box"><img src="{ip}{s["icon"]}" alt="{escape(s["name"])}" /></a>'
    ...
    for p in projects:
        ...
        img = f'<img src="{ip}{p["thumbnail"]}" alt="{title} thumbnail" />'
    ...
    doc = fonts_and_head(meta["title"], ip, extra) + f"""<body>
  <nav class="navigation">
    <a href="/" class="logo-link" aria-current="page">
      <img src="{ip}{SITE['logo']}" width="75" alt="ZLK logo" class="logo-image" />
    </a>
  </nav>
...
# ---------- case study blocks ----------
def render_block(b, ip):
    if b["type"] == "feature_img":
        return (f'<section class="section feature-img"><div class="container">'
                f'<img class="detail-header-image img-desktop" src="{ip}{b["desktop"]}" alt="" loading="lazy" /> '
                f'<img class="detail-header-image img-mobile" src="{ip}{b["mobile"]}" alt="" loading="lazy" />'
                f'</div></section>')
    if b["type"] == "imgs":
        ts = b.get("top_spacing_index")
        parts = ['<section class="section feature-img"><div class="container">']
        for i, d in enumerate(b["desktop"]):
            extra = " top-spacing" if ts == i else ""
            parts.append(f'<img class="detail-header-image img-desktop{extra}" src="{ip}{d}" alt="" loading="lazy" />')
        for m in b["mobile"]:
            parts.append(f'<img class="detail-header-image img-mobile" src="{ip}{m}" alt="" loading="lazy" />')
        parts.append('</div></section>')
        return "\n      ".join(parts)
    if b["type"] == "desc":
        # body may contain intentional inline HTML (<br>, <strong>, <em>) — kept as-is
        return (f'<section class="section"><div class="container">\n'
                f'        <div class="project-description-grid">\n'
                f'          <div><div class="position-name-text">{escape(b["heading"])}</div></div>\n'
                f'          <div class="paragraph-light">{b["body"]}</div>\n'
                f'        </div>\n      </div></section>')
    if b["type"] == "launch_center":
        return (f'<section class="launch-centered">\n'
...
"""
    ...
    blocks_html = "\n      ".join(render_block(b, ip) for b in p["blocks"])

    doc = fonts_and_head(p["title"], ip, extra) + f"""<body>
  <nav class="navigation">
    <a href="../../" class="logo-link"><img src="{ip}{SITE['logo']}" width="75" alt="ZLK logo" class="logo-image" /></a>
  </nav>
